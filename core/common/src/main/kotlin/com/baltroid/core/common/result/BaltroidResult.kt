...
sealed interface BaltroidResult<out T> {
    data class Loading<T>(val value: T? = null) : BaltroidResult<T> {
        override fun toString(): String = "Loading($value)"
    }

    sealed interface Success<out T> : BaltroidResult<T> {
        val value: T
        ...
        ) : Success<T>, com.baltroid.core.common.result.HttpResponse {
            override fun toString(): String =
                "HttpResponse($value, $statusCode, $statusMessage, $url)"
        }
    }

    sealed interface Failure<out T> : BaltroidResult<T> {
        val error: Throwable
        val value: T?
